...
export const getDashboardStats = async (req: Request, res: Response) => {
  try {
    const [
      ...
      Mood.countDocuments({ isDeleted: { $ne: true } }),
      Activity.countDocuments({ isDeleted: { $ne: true } }),
      ChatSession.countDocuments(),
      ReflectionReport.countDocuments(),
      User.countDocuments({
        role: "user",
...
export const getMoodTrends = async (req: Request, res: Response) => {
  try {
    ...
    const scoreDistribution = await Mood.aggregate([
      {
        $match: {
          timestamp: { $gte: startDate },
          isDeleted: { $ne: true },
...
export const getReportStats = async (req: Request, res: Response) => {
  try {
    ...
    const reportsByPeriod = await ReflectionReport.aggregate([
      {
        $match: {
          createdAt: { $gte: startDate },
        },
    ...
    const dailyReports = await ReflectionReport.aggregate([
      {
        $match: {
          createdAt: { $gte: startDate },
        },
 ...
 */
export const getActivityBlueprints = async (req: Request, res: Response) => {
  try {
    // Get unique activity types with counts
    const blueprints = await Activity.aggregate([
