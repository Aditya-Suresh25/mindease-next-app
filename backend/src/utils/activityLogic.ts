import { ACTIVITY_BLUEPRINTS, ActivityBlueprint } from "../data/blueprints";

type MoodCategory = "very_low" | "low" | "neutral" | "good" | "great";
...
const getMoodCategory = (score: number): MoodCategory => {
...
const getReasonTemplate = (category: MoodCategory, intensity: number): string => {
    if (intensity >= 4) {
...
export const selectActivities = (input: LogicInput): LogicOutput => {
    ...
    let candidates = ACTIVITY_BLUEPRINTS.filter(a =>
        a.suitableMoodCategories.includes(category)
    );
    ...
    if (input.intensity >= 4) {
        ...
        if (calming.length > 0) candidates = calming;
    }
